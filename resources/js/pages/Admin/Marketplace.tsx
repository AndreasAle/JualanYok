...
export default function MarketplaceAdmin({ products, filters, stats, configuration }: { products: Paginated<Listing>; filters: { q?: string; status?: string }; stats: Record<string, number>; configuration: Record<string, number> }) {
    ...
    return <DashboardLayout title="Marketplace" area="admin">
        ...
        <div className="mt-5 space-y-3">{products.data.map((product) => <article key={product.id} className="grid gap-4 rounded-2xl border border-line bg-white p-4 shadow-card md:grid-cols-[72px_1fr_auto] md:items-center"><div className="size-[72px] overflow-hidden rounded-xl bg-violet-50">{product.thumbnail_url ? <img src={product.thumbnail_url} alt="" className="size-full object-cover" /> : <Boxes className="m-6 size-6 text-violet-500" />}</div><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><Badge tone={tone[product.status]}>{product.status_label}</Badge><span className="text-[11px] font-bold text-muted">{product.type}</span><span className="text-[11px] font-bold text-muted">Kualitas {product.quality_score}/100</span></div><h2 className="mt-2 truncate text-sm font-semibold">{product.name}</h2><p className="mt-1 text-xs text-muted">{product.creator.name} · @{product.creator.username} · {product.category ?? 'Tanpa kategori'} · {formatIDR(product.price)}</p>{product.reason && <p className="mt-2 text-xs text-rose-600">Alasan terakhir: {product.reason}</p>}</div><div className="flex flex-wrap items-center gap-2 md:justify-end"><Link href={product.storefront_url} target="_blank" className="inline-flex h-9 items-center gap-1 rounded-xl border border-line px-3 text-[11px] font-bold">Lihat <ExternalLink className="size-3" /></Link>{product.status === 'APPROVED' && <button onClick={() => router.post(`/admin/marketplace/produk/${product.id}/unggulkan`, {})} className="inline-flex h-9 items-center gap-1 rounded-xl border border-violet-200 px-3 text-[11px] font-bold text-violet-700"><Sparkles className="size-3" /> Unggulkan</button>}<Button size="sm" onClick={() => setReview(product)}>{product.status === 'PENDING_REVIEW' ? 'Review' : 'Ubah status'}</Button></div></article>)}{products.data.length === 0 && <div className="rounded-2xl border border-dashed border-line bg-white p-16 text-center"><Flag className="mx-auto size-8 text-muted" /><p className="mt-3 font-semibold">Tidak ada listing pada filter ini</p></div>}</div>
