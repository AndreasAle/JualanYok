...
export default function NotificationBell({ area }: { area: DashboardArea }) {
    ...
    return <div ref={root} className="relative">
        ...
        >
            ...
            {notifications.unread_count > 0 && <span className="absolute -right-0.5 -top-0.5 grid min-w-4.5 place-items-center rounded-full bg-rose-500 px-1 text-[11px] font-bold leading-[18px] text-white ring-2 ring-white">
        ...
        {open && <div className="absolute right-0 top-12 z-50 w-[min(23rem,calc(100vw-1.5rem))] overflow-hidden rounded-2xl border border-line bg-surface shadow-lift">
            <div className="flex items-start justify-between border-b border-line px-4 py-3.5">
                <div><p className="text-sm font-semibold">Notifikasi</p><p className="mt-0.5 text-[11px] text-muted">{notifications.unread_count} belum dibaca · {notifications.action_count} perlu tindakan</p></div>
            ...
            <div className="flex items-center gap-1 border-b border-line px-3 py-2">
                ...
                {notifications.unread_count > 0 && <button type="button" onClick={markAllRead} className="ml-auto flex items-center gap-1 text-[11px] font-bold text-violet-700"><CheckCheck className="size-3.5" /> Baca semua</button>}
            </div>
            {items.length === 0 ? <div className="px-6 py-10 text-center"><span className="mx-auto grid size-11 place-items-center rounded-2xl bg-surface-2 text-muted"><Bell className="size-5" /></span><p className="mt-3 text-sm font-bold">Sudah beres</p><p className="mt-1 text-xs text-muted">Tidak ada notifikasi pada bagian ini.</p></div> : <ul className="max-h-[25rem] overflow-y-auto">
                {items.map((item) => <li key={item.id} className="border-b border-line last:border-0">
                    <a href={item.open_url} className="flex gap-3 px-4 py-3 transition hover:bg-surface-2">
                        ...
                        <span className="min-w-0 flex-1"><span className="flex items-start gap-2"><span className="line-clamp-1 flex-1 text-xs font-semibold">{item.title}</span>{item.action_required && !item.is_resolved && <span className="mt-1 size-2 shrink-0 rounded-full bg-amber-500" />}</span><span className="mt-1 line-clamp-2 block text-[11px] leading-4 text-muted">{item.message}</span><span className="mt-1.5 flex items-center gap-2 text-[11px] font-semibold text-muted"><span>{item.created_at_human}</span>{(item.group_count ?? 1) > 1 && <span className="rounded-full bg-surface-2 px-1.5 py-0.5">{item.group_count} pembaruan</span>}</span></span>
                    </a>
                </li>)}
            </ul>}
            <Link href={`${notifications.index_url}?area=${area}`} onClick={() => setOpen(false)} className="block border-t border-line px-4 py-3 text-center text-xs font-semibold text-violet-700 hover:bg-violet-50">Lihat semua & atur email</Link>
        </div>}
    </div>;
}
